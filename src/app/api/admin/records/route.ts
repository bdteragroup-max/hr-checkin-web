...
export async function GET(req: Request) {
    try {
        ...
        const emps = await prisma.employees.findMany({
            where: { is_active: true }, // Usually records only focus on active employees, or maybe all? Let's use all to be safe for historical data, but optionally filter by frontend.
            select: {
                emp_id: true,
                name: true,
                branch_id: true,
